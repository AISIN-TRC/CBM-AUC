...
def binary_accuracy: 
    compute binary accuracy of task (not use in our case, is not modified)
def accuracy: 
    compute accyracy of task (is not modified)
def plot_losses: 
    make figures of losses (is not modified)
"""
class ClassificationTrainer():
    ...
    def concept_learning_loss_for_weak_supervision:
        compute losses of known concepts and discriminator
    Inputs:
        inputs_aux: auxilary output of inception v.3
        inputs: output of inception v.3
    ...
    """    
    def concept_learning_loss_for_weak_supervision(self, inputs_aux, inputs, all_losses, concepts, cbm, senn, epoch):

        # compute predicted known concepts by inputs and aux. inputs
        # real and real_aux use the discriminator's loss
        hh_labeled_list, h_x, real = self.model.conceptizer(inputs)
        hh_labeled_list_aux, h_x_aux, real_aux = self.model_aux.conceptizer(inputs_aux)

        if not senn:

            # compute losses of known concepts
            labeled_loss = F.mse_loss(hh_labeled_list,concepts)
            labeled_loss_aux = F.mse_loss(hh_labeled_list_aux,concepts)

            """
            weighted add two losses 
            weight of output loss is 1.0 and weight of aux. loss is 0.4
            These weights are the same to the CBM's authors
            """
            labeled_loss = (labeled_loss + 0.4*labeled_loss_aux)*len(concepts[0])
        ...
        NOTE1: if you use CBM's model, skip because CBM does not use followings
        NOTE2: if you use DeepInfoMax loss, please remove the comment outs of info_loss and info_loss_aux
        """
        if not cbm:            
            ...
            """
            compute loss
            if you use DeepInfoMax loss, please remove the comment outs of info_loss and info_loss_aux
            """
            info_loss = self.gamma*(F.mse_loss(real, labs_real) + F.mse_loss(fake, labs_fake))
            #info_loss = self.gamma*(F.softplus(-real).mean()+F.softplus(fake).mean())

            # following the aux. loss

            # select random index
            rand_index = torch.randperm(h_x.size()[0]).cuda(self.device)

            # compute fake data (concate[rand_inputs, known_conepts, unknown_concepts]) and output of the discriminator
            fake = self.model_aux.conceptizer.decode(inputs_aux[rand_index], hh_labeled_list_aux, h_x_aux)

            """
            compute aux. loss
            if you use DeepInfoMax loss, please remove the comment outs of info_loss and info_loss_aux
            """
            info_loss_aux = self.gamma*(F.mse_loss(real_aux, labs_real) + F.mse_loss(fake, labs_fake))
            #info_loss_aux = self.gamma*(F.softplus(-real_aux).mean()+F.softplus(fake).mean())
            ...
            These weights are the same to the CBM's authors
            """
            info_loss = info_loss + 0.4*info_loss_aux
    ...
    def validate(self, val_loader, fold = None):
        ...
        for i, (inputs, targets, concepts) in enumerate(val_loader):
            ...
            # compute output        
            aux_latent,pretrained_out = self.pretrained_model(inputs)
        ...
        for i, (inputs, targets, concepts) in enumerate(test_loader):
            ...
            if not self.args.senn:
                ...
                for j in range(len(targets)):
                    fp.write("%d,%d,"%(targets[j],pred_labels[j]))
...
def compute_conceptizer_jacobian:
    compute jacobian of output (not modified)
def compute_conceptizer_jacobian_aux:
    compute jacobian of aux. output (added)
"""        
class GradPenaltyTrainer(ClassificationTrainer):
    ...
    Returns:
        None
    """
    def __init__(self, model, model_aux, pretrained_model, args, device):
        ...
        if args.opt == 'adam':
            ...
            self.aux_optimizer = optim.Adam(self.model_aux.parameters(), lr= args.lr, betas=optim_betas)
            ...
            self.aux_optimizer = optim.RMSprop(self.model_aux.parameters(), lr = args.lr)
            ...
            self.aux_optimizer = optim.SGD(self.model_aux.parameters(), lr = args.lr, weight_decay = args.weight_decay, momentum=0.9)
        ...
        self.aux_scheduler = torch.optim.lr_scheduler.StepLR(self.aux_optimizer, step_size=20, gamma=0.1)
    ...
    def train_batch(self, inputs, targets, concepts, epoch):
        ...
        aux_pred = self.model_aux(aux_latent)
        
        # Calculate loss
        pred_loss1 = self.prediction_criterion(pred, targets)
        pred_loss2 = self.prediction_criterion(aux_pred, targets)
        ...
        These weights are the same to the CBM's authors
        """
        pred_loss = 1.0*pred_loss1 + 0.4*pred_loss2
        
        # save loss (only value) to the all_losses list
        all_losses = {'prediction': pred_loss.cpu().data.numpy()}
            
        # compute loss of known concets and discriminator
        h_loss, hh_labeled = self.concept_learning_loss_for_weak_supervision(aux_latent, pretrained_out, all_losses, concepts, self.args.cbm, self.args.senn, epoch)

        # to be simplify, redifine
        inputs = pretrained_out
        inputs_aux = aux_latent
        ...
        dF_aux = compute_jacobian(inputs_aux, aux_pred, self.device)

        dH  = self.compute_conceptizer_jacobian(inputs)
        prod = torch.bmm(dH, self.model.thetas)

        dH_aux  = self.compute_conceptizer_jacobian_aux(inputs_aux)
        prod_aux = torch.bmm(dH_aux, self.model_aux.thetas)

        ## For V2:
        grad_penalty = (prod - dF).norm(self.norm) 
        grad_penalty_aux = (prod_aux - dF_aux).norm(self.norm)
        ...
        These weights are the same to the CBM's authors
        """
        grad_penalty = 1.0*grad_penalty + 0.4*grad_penalty_aux
    ...
    """
    def compute_conceptizer_jacobian_aux:
        compute jacobian of aux. output
    Inputs:
        x: output of encoder
    Returns:
        Jh: jacobian
    NOTE: newly added (almost same to the compute_conceptizer_jacobian). only difference is to first line (h=...)
    """
    def compute_conceptizer_jacobian_aux(self, x):
        h = self.model_aux.concepts
        Jh = compute_jacobian(x, h.squeeze(), self.device)
        assert list(Jh.size()) == [x.size(0), x.view(x.size(0),-1).size(1), h.size(1)]
        return Jh
