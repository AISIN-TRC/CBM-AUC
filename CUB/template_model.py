...
model_urls = {
    ...
    'downloaded': 'models/inception_v3_google-1a9a5a14.pth',
...
def inception_v3(pretrained, freeze, **kwargs):
    ...
    if pretrained:
        ...
        if os.path.exists(model_urls.get('downloaded')):
            ...
            #print(model.Conv2d_1a_3x3.conv.weight[0])
        else:
...
class Inception3(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.aux_logits:
        #if self.training and self.aux_logits:
            #out_comb = torch.cat([out_aux,out_last], dim=1)
            #return out, out_comb
            return out_latent, out_last
        else:
            return out_last
